...
public static class Helpers<T> where T : PKM, new()
{
    ...
    public static Task<ProcessedPokemonResult<T>> ProcessShowdownSetAsync(string content, bool ignoreAutoOT = false)
    {
        ...
        if (isEgg)
        {
            ...
            if (pkm != null)
                ApplySpecifiedBallToEgg(pkm, content);
        }
        else
        {
    ...
    /// <summary>Delegates to <see cref="EggBallHelper.ApplySpecifiedBall"/>.</summary>
    public static void ApplySpecifiedBallToEgg(PKM pkm, string content) => EggBallHelper.ApplySpecifiedBall(pkm, content);
